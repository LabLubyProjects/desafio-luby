...
export default class GetAllEmployeesUseCase extends BaseEmployeeRelatedUseCase {
  async handle(): Promise<OutputEmployee[]> {
    ...
    const outputGetEmployees = employees.map(employee => ({ id: employee.id, cpf: employee.cpf, name: employee.name, email: employee.email, biography: employee.biography, type: employee.type }))
    return outputGetEmployees;
  }
}