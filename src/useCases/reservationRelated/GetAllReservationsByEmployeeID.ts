...
export default class GetAllReservationsByEmployeeID extends BaseReservationRelatedUseCase {
  async handle(
    inputGetAllReservationsByEmployee: InputGetAllReservationsByEmployee
  ): Promise<OutputReservation[]> {
    const reservations = await this.reservationRepository.getByEmployeeID(
      inputGetAllReservationsByEmployee.employeeID,
      inputGetAllReservationsByEmployee.pageNumber,
      inputGetAllReservationsByEmployee.pageSize
    );
    const employee = await this.employeeRepository.getByID(
      inputGetAllReservationsByEmployee.employeeID
    );
    const outputReservations: OutputReservation[] = await Promise.all(
