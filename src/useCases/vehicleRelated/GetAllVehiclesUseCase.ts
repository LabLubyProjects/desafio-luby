...
export default class GetAllVehiclesUseCase extends BaseVehicleRelatedUseCase {
  async handle(
    inputGetAllVehicles: InputGetAllVehicles
  ): Promise<OutputVehicle[]> {
    const vehicles = await this.vehicleRepository.getAll(
      inputGetAllVehicles.pageNumber,
      inputGetAllVehicles.pageSize
    );
    const outputVehicles: OutputVehicle[] = vehicles.map((vehicle) => ({
      id: vehicle.id,
      brand: vehicle.brand,
      model: vehicle.model,
      year: vehicle.year,
