...
export default class GetVehicleByID extends BaseVehicleRelatedUseCase {
  async handle(inputGetVehicleByID: InputGetVehicleByID): Promise<OutputVehicle> {
    ...
    const outputVehicle: OutputVehicle = { id: vehicle.id, brand: vehicle.brand, model: vehicle.model, year: vehicle.year, km: vehicle.km, color: vehicle.color, chassi: vehicle.chassi, price: vehicle.price }; 
