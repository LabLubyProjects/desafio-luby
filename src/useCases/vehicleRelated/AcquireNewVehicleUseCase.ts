...
export default class AcquireNewVehicleUseCase extends BaseVehicleRelatedUseCase {
  async handle(inputAcquireNewVehicle: InputAcquireNewVehicle): Promise<OutputVehicle> {
    ...
    const outputVehicle: OutputVehicle = { id: newVehicle.id, brand: newVehicle.brand, model: newVehicle.model, year: newVehicle.year, km: newVehicle.km, color: newVehicle.color, chassi: newVehicle.chassi, price: newVehicle.price }; 
