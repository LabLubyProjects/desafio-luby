...
import {
  InputAcquireNewVehicle,
  InputDeleteVehicle,
  InputFilterVehicleByStatus,
  InputGetAllVehicles,
  InputGetVehicleByID,
  OutputVehicle,
} from '@src/useCases/vehicleRelated/VehicleIO';
...
export default class VehicleController extends BaseController {
  static async createVehicle(
    ...
    const input = new InputAcquireNewVehicle(
      sourceEmployeeID,
      brand,
      model,
      year,
    ...
    return {
      id: await new AcquireNewVehicleUseCase(
        vehicleRepository,
        employeeRepository
      ).handle(input),
    };
  ...
  static async getAllVehicles(
    ...
    else input = new InputGetAllVehicles();

    await BaseController.validateInput(input);

    return new GetAllVehiclesUseCase(
      vehicleRepository,
      employeeRepository
    ).handle(input);
  }
  ...
  static async getVehiclesByID(
    ...
    return new GetVehicleByID(vehicleRepository, employeeRepository).handle(
      input
    );
  }
  ...
  static async deleteVehicle(
    ...
    return new DeleteVehicleUseCase(
      vehicleRepository,
      employeeRepository
    ).handle(input);
  }
  ...
  static async filterByStatus(
    ...
    if (page && size)
      input = new InputFilterVehicleByStatus(
        Number.parseInt(status),
        Number.parseInt(page),
        Number.parseInt(size)
      );
    else input = new InputFilterVehicleByStatus(Number.parseInt(status));

    await BaseController.validateInput(input);

    return new FilterVehicleByStatusUseCase(
      vehicleRepository,
      employeeRepository
    ).handle(input);
  }
