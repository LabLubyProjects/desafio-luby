...
import {
  ...
  InputReserve,
  OutputReservation,
} from '@src/useCases/reservationRelated/ReservationIO';
...
export default class ReservationController extends BaseController {
  ...
  static async getAllReservationsByEmployee(
    ...
    if (page && size)
      input = new InputGetAllReservationsByEmployee(
        employeeID,
        Number.parseInt(page),
        Number.parseInt(size)
      );
    else input = new InputGetAllReservationsByEmployee(employeeID);

    await BaseController.validateInput(input);

    return new GetAllReservationsByEmployeeID(
      reservationRepository,
      employeeRepository,
      vehicleRepository
    ).handle(input);
