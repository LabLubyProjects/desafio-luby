import VehicleImpl, { Vehicle, VehicleStatus } from "@src/domain/vehicle/Vehicle";
...
export default class SequelizeVehicleRepository implements VehicleRepository {
  ...
  async getByID(id: string): Promise<Vehicle | null> {
    ...
    return new VehicleImpl(vehicleFromDB.getDataValue('brand'), vehicleFromDB.getDataValue('model'), vehicleFromDB.getDataValue('year'), vehicleFromDB.getDataValue('km'), vehicleFromDB.getDataValue('color'), vehicleFromDB.getDataValue('chassi'), vehicleFromDB.getDataValue('price'), vehicleFromDB.getDataValue('status'),vehicleFromDB.getDataValue('id'));
  }
  async create(newVehicle: Vehicle): Promise<string> {
    await VehicleModel.create({ brand: newVehicle.brand, model: newVehicle.model, year: newVehicle.year, km: newVehicle.km, color: newVehicle.color, chassi: newVehicle.chassi, price: newVehicle.price, status: newVehicle.status, id: newVehicle.id });
  ...
  async updateStatus(id: string, newStatus: VehicleStatus): Promise<void> {
    await VehicleModel.update({ status: newStatus }, {
      where: {
        id: id
      }
    });
